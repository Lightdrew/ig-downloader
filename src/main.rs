async fn fetch_reel(url: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if response.status().is_success() {
                ...
                loop {
                    ...
                    let video = match video {
                        Some(video) => video
                            ...
                            .or_else(|| {
                                video.get("xdt_api__v1__clips__home__no__login__connection_v2")
                            }),
                        None => continue,
                    };
                    ...
                    match video {
                        Some(video) => {
                            ...
                            let url = video["video_versions"][0]["url"]
                                .to_string()
                                .replace("\"", "");
                            ...
                            if video_response.status().is_success() {
                                ...
                                if !tokio::fs::read_dir("download").await.is_ok() 
                                {
                                    match tokio::fs::create_dir("download").await {
                                        Err(e) => {
                                ...
                                return Err(Box::new(std::io::Error::new(
                                    std::io::ErrorKind::Other,
                                    format!(
                                        "Failed to download video: Request failed ({})",
        ...
        return Err(Box::new(std::io::Error::new(
            std::io::ErrorKind::Other,
            format!(
                "Failed to fetch reel: Request failed ({})",
                response.status()
            ),
        )));
    }
